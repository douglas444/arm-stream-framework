...
public class DynamicConfusionMatrix {
    ...
    public boolean isLabelUnknown(final Integer label) {
    ...
    private HashMap<Integer, List<Integer>> calculateNoveltyAssociationByRow() {
    ...
    public int truePositive(final int label, final HashMap<Integer, List<Integer>> noveltyAssociationByRow) {
    ...
    public int falsePositive(final int label, final HashMap<Integer, List<Integer>> noveltyAssociationByRow) {
    ...
    public int falseNegative(final int label, final HashMap<Integer, List<Integer>> noveltyAssociationByRow) {
    ...
    public int trueNegative(final int label, final HashMap<Integer, List<Integer>> noveltyAssociationByRow) {

        return this.rowLabels.stream()
                .filter(rowLabel -> rowLabel != label)
                .map(rowLabel -> this.truePositive(rowLabel, noveltyAssociationByRow))
                .reduce(0, Integer::sum);

    }

    public int numberOfExplainedSamplesByLabel(final int label) {
    ...
    public int numberOfExplainedSamples() {
        return this.rowLabels.stream().map(this::numberOfExplainedSamplesByLabel).reduce(0, Integer::sum);
    }

    public double combinedError() {
        ...
        sum += this.rowLabels.stream().map(rowLabel -> {
            ...
            final int fp = this.falsePositive(rowLabel, association);
            final int fn = this.falseNegative(rowLabel, association);
            final int tn = this.trueNegative(rowLabel, association);
            final int tp = this.truePositive(rowLabel, association);

            final int numberOfExplainedSamples = this.numberOfExplainedSamplesByLabel(rowLabel);
    ...
    public double unknownRate() {

        return this.rowLabels.stream()
                .map(rowLabel -> {

                    final double unexplained = this.unknownColumn.get(this.rowIndexByLabel.get(rowLabel));
                    final double explained = this.numberOfExplainedSamplesByLabel(rowLabel);
