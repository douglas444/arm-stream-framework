...
class DependencyFinder {
    ...
    static Set<Class<?>> getDependencies(Class<?> from) throws ImplementationLoaderException {
        ...
        try (InputStream is = from.getResourceAsStream(
                '/' + from.getName().replace('.', '/') + ".class")) {

            if (is == null) {
                throw new ImplementationLoaderException(MessageFormat.format(ERROR_FINDING_DEPENDENCIES, from.getName()));
            }
            ...
            throw new ImplementationLoaderException(MessageFormat.format(ERROR_FINDING_DEPENDENCIES, from.getName()), e);
        }
    ...
    private static Set<String> getDependencies(ByteBuffer bb) {
        ...
        for (int c = 1; c < numC; c++) {
            ...
            switch (bb.get()) {
                ...
                case CONSTANT_Utf8:
                    ...
                    if (clazz.get(c)) {
                        if (bb.get(bb.position()) == '[') {
                            s = true;
                        } else {
                            addName(names, bb, strStart, strSize);
                        }
                    }

                    if (s) {
                        addNames(names, bb, strStart, strSize);
                    }
    ...
    private static void addName(HashSet<String> names, ByteBuffer src, int s, int strSize) {
    ...
    private static void addNames(HashSet<String> names, ByteBuffer bb, int s, int l) {
        final int e = s + l;
        for (; s < e; s++) {
            if (bb.get(s) == 'L') {
                int p = s + 1;
